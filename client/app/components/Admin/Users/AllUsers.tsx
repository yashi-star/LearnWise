...
const AllUsers:FC<Props> = ({isTeam}) => {
    ...
    {field:'delete',headerName:'Delete', flex:0.2,
            ...
            {field:'email_action',headerName:'Email',
             flex:0.2,
                renderCell:(params:any)=>{
                    return (
                        <>
    ...
    if(isTeam){
{
    const newData=data && data.users.filter((item:any) => item.role==="admin");
        ...
        newData?.forEach((item: { _id: string; name: string; email: string; role: string; courses: any[]; createdAt: string }) =>{
            rows.push({
        ...
        data?.users.forEach((item: { _id: string; name: string; email: string; role: string; courses: any[]; createdAt: string }) => {
            rows.push({
                id: item._id,
                name: item.name,
                email: item.email,
            ...
            isLoading ? (
                ...
                <Box m="20px">
                    <div className="w-full flex justify-end">
                        <div className={`${styles.button} !w-[200px] dark:bg-[#5780c7] !h-[35px] dark:border-[#ffffff6c]`}
                ...
                aria-describedby="modal-modal-description">
                    <Box className="absolute top-[50%] -translate-x-1/2 ">
                        ...
                        <div className='flex w-full items-center justify-between mb-6 '>
                            <div className={`${styles.button} !w-[120px] h-[30px]  bg-[#4180b3]`}
                            onClick={() => setOpen(!open)}
                            >
                            Cancel
                            </div>
                            <div className={`${styles.button}  !w-[120px] h-[30px] bg-[#ba4450]`}
