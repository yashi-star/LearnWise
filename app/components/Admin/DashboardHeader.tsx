...
const DashboardHeader: FC<Props> = ({ open = false, setOpen }) => {
  ...
  useEffect(() => {
    setAudio(
      new Audio(
        "https://res.cloudinary.com/dhcc2rwis/video/upload/v1739600695/notification-2-269292_iqpgrz.mp3"
      )
    );
  }, []);
        ...
        <div className="w-[350px] h-[50vh] dark:bg-[#111C43] bg-white shadow-xl absolute top-16 z-10 rounded">
          ...
          {notifications.length > 0 ? (
            notifications.map((item: any, index: number) => (
              <div
                key={index}
                className="dark:bg-[#2d3a4ea1] bg-[#f3f4f6] font-Poppins border-b dark:border-b-[#ffffff47] border-b-[#d1d5db]"
              >
